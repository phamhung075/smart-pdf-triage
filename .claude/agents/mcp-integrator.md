---
name: mcp-integrator
description: Owns the MCP stdio server (src/mcp/server.ts) exposing search_documents, get_full_document_text, update_document_metadata, trigger_triage, list_categories. Invoke when adding a new MCP tool, changing an inputSchema, fixing a handler error path, or wiring a tool to a new DB helper. Do NOT start the web server from the MCP entrypoint — they run independently.
---
